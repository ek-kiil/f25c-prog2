...
> ```java
> bookings.stream()
>         .sorted(Comparator.comparingInt(booking -> actualTravelMinutes(booking))
...
> ```
>
> For vores egne statiske hjælpermetoder bruger vi i dette ark den her form:
>
> ```java
...
<details>
<summary>Hint</summary>
Brug hjælpermetoden inde i lambdaen, fx:

```java
booking -> actualTravelMinutes(booking)
```

Det virker godt i fx `sorted(...)`.
</details>
...
Du kan fx bruge hjælpermetoden sådan her:

```java
booking -> pricePer100Km(booking)
```
...
Inde i `averagingInt(...)` kan du fx skrive:
...
Tænk opgaven i tre faser:

1. filtrér til de relevante ture
2. gruppér efter flyselskab
3. stream over grupperne og byg resultatet
</details>

<details>
<summary>Hint 2</summary>
Lav først et `Map<String, List<FlightBooking>>`.

Når du bagefter streamer over `entrySet()`, er:

- `entry.getKey()` navnet på flyselskabet
- `entry.getValue()` alle ture for det flyselskab
</details>
...
Tænk på én airlinegruppe ad gangen.

For gruppens liste af ture skal du kunne finde:

- antal ture
- gennemsnitlig rating
- gennemsnitlig pris pr. 100 km

Hvis det bliver for rodet inde i `.map(...)`, må du gerne flytte det ud i en hjælpermetode som tager airline-navnet og gruppens ture.
</details>

<details>
<summary>Hint 4</summary>
En mulig strategi er:

1. filtrér først til weekendture med faktisk rejsetid på højst 240 minutter
2. gruppér de ture efter flyselskab
3. stream over grupperne med `entrySet().stream()`
4. brug gruppens liste til at regne antal ture, gennemsnitlig rating og gennemsnitlig pris pr. 100 km ud
5. sortér grupperne efter højeste rating og derefter laveste pris pr. 100 km
6. map til den tekstlinje du vil vise
</details>
...
<details>
<summary>Hint 1</summary>
Tænk opgaven sådan her:

1. gruppér først efter kunde
2. behold kun de kundegrupper der opfylder reglerne
3. sortér og map til tekst til sidst
</details>

<details>
<summary>Hint 2</summary>
For hver kundegruppe skal du mindst regne disse ting ud:

- antal forskellige lande
- gennemsnitlig rating
- samlet forsinkelse
- samlet forbrug
</details>
...
De andre værdier finder du på samme kundegruppe:
...
1. gruppér alle bookinger efter kunde
2. stream over kundegrupperne
3. brug hver gruppes liste til at finde antal forskellige lande, gennemsnitlig rating, samlet forsinkelse og samlet forbrug
4. behold kun de kunder der opfylder alle tre regler
5. sortér de tilbageværende kunder efter lavest samlet forbrug
6. map til tekst

Hvis `.map(...)` bliver meget lang, må du gerne flytte formatteringen ud i en hjælpermetode som tager kundenavn og gruppens ture.
</details>
...
<details>
<summary>Hint 1</summary>
Tænk opgaven i fire faser:

1. filtrér til de relevante ture
2. gruppér efter flyselskab
3. find top 3 i hver gruppe
4. regn videre kun på de 3 ture
</details>
...
<details>
<summary>Hint 3</summary>
For hver airlinegruppe skal du først lave en ny liste med de 3 bedste ture.

Det kan du gøre ved at:

1. streame over gruppens ture
2. sortere dem
3. bruge `limit(3)`
4. gemme resultatet som `topTrips`

Det er den liste du bagefter skal regne rating, pris og pris pr. 100 km ud fra.
</details>
...
1. filtrér først til weekendture med faktisk rejsetid på højst 240 minutter
2. gruppér de ture efter flyselskab
3. gå igennem hver airlinegruppe og lav en ny liste `topTrips` med de 3 bedste ture
4. kassér de grupper hvor `topTrips` har færre end 3 ture
5. regn gennemsnitlig rating, samlet pris og gennemsnitlig pris pr. 100 km ud fra `topTrips`
6. sortér resultatet efter rating, derefter pris pr. 100 km, derefter samlet pris
7. map til tekst

Hvis koden bliver meget tung i `.map(...)`, må du gerne bruge en hjælpermetode til at formatere resultatet for én airlinegruppe.
</details>
