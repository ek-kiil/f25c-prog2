...
**Today we work in pairs.**

- **one person starts work on the frontend**
- **one person starts on the backend**
- later on we gather forces
- along the way we discuss what we are doing

<br>

The important thing today is that the frontend arrays and the JSON from the backend use the **same structure**.
...
<details>
  <summary><b>Extra extra hint (First try for a bit and ask your frontend partner what they expect :)</b></summary>
...
<details>
  <summary><b>Extra extra hint (Try to ask the frontend for assistance :)</b></summary>


Now we almost have the full controller, we just need to map the lists from the services:
...
As we first introduce streams in the next two lectures, a plain loop is fine here. 
