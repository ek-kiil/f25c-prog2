...
We are building a Spring Boot application that will act as the backend for our Region and Municipality data. We start by creating a Spring project and choosing the dependencies we need.
...
4. **Setting:** `spring.jpa.hibernate.ddl-auto=update`. This tells Hibernate (the JPA provider) to automatically check your `Region` entity and create/update the table in the database when the application starts, why do we use **update** today and not create-drop? `update` keeps existing data between restarts. `create-drop` deletes the tables when the app stops, which is fine for tiny demos but annoying when we want to inspect imported data in MySQL.
...
**Verification:** Run your application. Open your browser and go to `http://localhost:8080/regioner/hent`.
...
- **Why Bidirectional?** In a database, only the child (`kommune`) knows who its parent is. The parent (`region`) knows nothing. By adding a list to the Region class, we make the relationship **Bidirectional** in Java. This allows us to navigate from a `Region` object to its kommuner in our code.
...
- **Should we use cascade here?** Not for this guide. We save `Region` and `Kommune` through separate repositories, so the relation should be clear without letting Region control the lifecycle of Kommune.
...
We now need to check if everything works and we can store some Kommuner - I really hope so :). Kommuner is not the most exciting thing to store, but using today's approach we can fetch, manipulate and later on show any data we want.

**Step 1: Run the Fetch**

1. Start application.
2. First, ensure Regions are loaded by visiting: `http://localhost:8080/regioner/hent`
3. Next, load the Municipalities by visiting: `http://localhost:8080/regioner/kommuner` (should match your controller mapping).
...
**Step 3: The "Trap" (I know we already tried this, this is the last time i swear)**. We have successfully ingested the data. But let's say we wanted to display a list of Regions to a user on a frontend.

If we were to create a simple endpoint like this:

Java

```
@GetMapping("/db")
public List<Region> getAllRegionsFromDatabase() {
...
### **Part 5: DTOs: avoid recursion and decouple the API**

A bidirectional relation is useful inside Java, but it is risky to return entities directly as JSON. `Region` points to `Kommune`, and `Kommune` points back to `Region`. JSON has no natural stopping point, so the response can become too large or end in infinite recursion.

A DTO lets us decide exactly what the frontend receives. It also decouples the API from the database model. If we later rename a field on an entity, add a relation, or change the external API, the frontend does not need to know as long as the DTO stays stable.

Example: a DTO for kommuner:

```java
public record KommuneResponse(
        String kode,
        String navn,
        String regionKode,
        String regionNavn
...
Then map from entity to DTO before returning data:

```java
private KommuneResponse toResponse(Kommune kommune) {
    return new KommuneResponse(
            kommune.getKode(),
            kommune.getNavn(),
            kommune.getRegion().getKode(),
            kommune.getRegion().getNavn()
    );
}
```

An endpoint can then return DTOs instead of entities:

```java
@GetMapping("/kommuner/db")
public List<KommuneResponse> getKommunerFromDatabase() {
    return kommuneRepository.findAll().stream()
            .map(this::toResponse)
            .toList();
}
```

Here the response contains the fields we chose. It does not expose the full `Kommune` entity, and it does not try to serialize the whole `Region -> Kommune -> Region` cycle.

**Questions:**

1. Which fields from `Kommune` are exposed in `KommuneResponse`?
2. Why do we include `regionKode` and `regionNavn` instead of the whole `Region` object?
3. What would the frontend need to change if the entity changed but the DTO stayed the same?
