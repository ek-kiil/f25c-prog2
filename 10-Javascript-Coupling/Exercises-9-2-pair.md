...
If you use IntelliJ's built-in server, the frontend may run on another origin than `http://localhost:5500`. In that case, update `@CrossOrigin(...)` so it allows the exact origin you are actually using.
...
Work on this together. The point here is to talk through how the data moves from backend to frontend.
...
<details>
  <summary><b>Extra extra hint (Talk it through together before clicking)</b></summary>
This is very close. The main change is that the old render calls should now be replaced with `loadAllData()`:
...
It makes sense to work together on the rest of the exercise, since the frontend and backend now depend directly on each other. If you prefer, you can split up again and meet in task 5.
...
<details>
  <summary><b>Extra extra hint (Try for a bit first, then discuss it with your partner)</b></summary>
...
This is very close. The last step is to call `loadAllData()` afterwards:
...
If something does not work, check these places:
...
If there is still time left, you can now make the status UI work in the browser.
...
This is very close. You still need to call `loadAllData()` and append the dropdown to the card:
